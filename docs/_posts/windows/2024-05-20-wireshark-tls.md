...
last_modified_at: 2026-08-22 23:16:00 +0800
categories: c-cpp
tags:
  - wireshark
  - tls
...
1. 通过wireshark官方文档[TLS解密]( https://wiki.wireshark.org/TLS#tls-decryption)知, wireshark解密TLSv1.3只能通过获取(Pre)-Master Secret方式.另外:
...
2. 如果编译libwebsockets.so时链接了静态的libssl.a和libcrypto.a, 没有链接libssl.so动态库, 则无法通过LD_PRELOAD的方式获取(Pre)-Master Secret.

3. 通过libwebsockets[官方仓库issue](https://github.com/warmcat/libwebsockets/issues/3087)知,  可以使用lws_klog_dump捕获打印(Pre)-Master Secret到环境变量SSLKEYLOGFILE设置的文件里.  
但是此功能**只在main分支及4.5.x分支**.
